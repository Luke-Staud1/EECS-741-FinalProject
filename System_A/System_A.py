...
def main():
    ...
    temp = []
    for i in range(1, 11):
        image2 = cv2.imread('../ProbeSet/subject' + str(i) + '_img2.pgm', cv2.COLOR_BGR2GRAY)         # Used for reading Tool Image in black and white --> https://opencv.org/
        for j in range(1, 11):
            image1 = cv2.imread('../GallerySet/subject' + str(j) + '_img1.pgm', cv2.COLOR_BGR2GRAY)         # Used for reading Tool Image in black and white --> https://opencv.org/
            temp.append(ComparePlots(image1, image2))
        score_matrix.append(temp) if len(temp) > 0 else []
        temp = []
